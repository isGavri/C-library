...
int main(void) {
  ...
  u32 num_notes = sizeof(mario) / sizeof(mario[0]);

  f32 duration = 0.0f;
  for (u32 i = 0; i < num_notes; i++) {
    duration += mario[i].dur;
  }
  ...
  for (u32 i = 0; i < num_sample; i++) {
    ...
    if (cur_note < num_notes) {
      y = 0.25f * sinf(t * mario[cur_note].freq * 2.0f * 3.1415926535f);

      if (t > cur_note_start + mario[cur_note].dur) {
        cur_note++;
        cur_note_start = t;
      }
    }
